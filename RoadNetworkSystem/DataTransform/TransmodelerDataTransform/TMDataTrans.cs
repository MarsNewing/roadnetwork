...
namespace RoadNetworkSystem.TransmodelerDataTransform
{
    class TMDataTrans
    {
        ...
        public TMDataTrans(OleDbConnection conn)
        {
            _conn = conn;
        }
        ...
        public static Boolean CreateTransmodelerdata(string apppath, string databasePath)
        {
            ...
            IFeatureClass pFeatClsNode = null;
            //IPoint NewpFeatClsNode;
            ...
            {
                if (featClass.AliasName == "Node")
                {
                    pFeatClsNode = featClass;
                }
            ...
            //1.获取当前坐标系，转换为TM坐标系
            //NewpFeatClsNode = Setcoorsystem(pFeatClsNode, pFeatClsLink, pFLayer1, pFLayer2);
            //NewpFeatClsLink = Setcoorsystem(pFeatClsNode, pFeatClsLink, pFLayer1, pFLayer2);
            //2.根据已有数据生成TM数据表
        ...
        private static void CreateTMData(IFeatureClass pFeatClsNode, IFeatureClass pFeatClsLink)
        {
            ...
            while (pFeatLink != null)//遍历弧段
            {
                ...
                m++;
                f = pFeatClsLink.FindField("LinkID");//当前弧段的LinkID
                linkid = Convert.ToInt32(pFeatLink.get_Value(f));
                a = pFeatClsLink.FindField("Shape_Length");//当前弧段的Shape_Length
                len = Convert.ToInt32(Convert.ToDouble(pFeatLink.get_Value(a)));
                b = pFeatClsLink.FindField("FNodeID");//当前弧段的FNodeID
                ANode = Convert.ToInt32(pFeatLink.get_Value(b));
                c = pFeatClsLink.FindField("TNodeID");//当前弧段的TNodeID
                BNode = Convert.ToInt32(pFeatLink.get_Value(c));
                d = pFeatClsLink.FindField("Dir");//当前弧段的Dir
                dir = Convert.ToInt32(pFeatLink.get_Value(d));
                if (dir == 2)
                    dir = 0;

                string Str_1 = "select LaneNum from Arc where LinkID=" + linkid + "and Dir =1";
                OleDbCommand Com_1 = new OleDbCommand(Str_1, _conn);
                lanesAB = Convert.ToInt32(Com_1.ExecuteScalar());

                Str_1 = "select LaneNum from Arc where LinkID=" + linkid + "and Dir =-1";
            ...
            {
                n++;
                laneid = Convert.ToInt32(reader["LaneID"]);
                lanearcid = Convert.ToInt32(reader["ArcID"]);
                lanepos = Convert.ToInt32(reader["Position"]);
                lanepos = lanepos - 1;
                lanechange = Convert.ToString(reader["Change"]);
                string Str_1 = "select LinkID from Arc where ArcID=" + lanearcid;
                OleDbCommand Com_1 = new OleDbCommand(Str_1, _conn);
                laneseg = Convert.ToInt32(Com_1.ExecuteScalar());
                Str_1 = "select ID from Transmodeler_Segments where Link=" + laneseg;
                Com_1 = new OleDbCommand(Str_1, _conn);
                laneseg = Convert.ToInt32(Com_1.ExecuteScalar());
                Str_1 = "select Dir from Arc where ArcID=" + lanearcid;
                Com_1 = new OleDbCommand(Str_1, _conn);
                lanedir = Convert.ToInt32(Com_1.ExecuteScalar());
                Str_1 = "select LaneNum from Arc where ArcID=" + lanearcid;
                Com_1 = new OleDbCommand(Str_1, _conn);
                ...
                if (lanepos == 0)
                    ...
                    laneside = "";
                string strSql1 = string.Format("select * from LaneConnectors where fromLaneID = {0:G}", laneid);
                ...
                if (readDB.HasRows)
                {
                    while (readDB.Read())
                    {
                        turndir = Convert.ToString(readDB["TurningDir"]);
            ...
            while (readerLC.Read())
            {
                lcid = Convert.ToInt32(readerLC["ConnectorID"]);
                ul = Convert.ToInt32(readerLC["fromLaneID"]);
                dl = Convert.ToInt32(readerLC["toLaneID"]);
                dirlc = Convert.ToString(readerLC["TurningDir"]);
