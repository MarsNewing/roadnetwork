    ...
    class TurnArrowService
    {
        ...
        public void CreateExitArcArrow(IFeatureClass pFeaClsLane, int arcID)
        {
            ...
            while (pFeature != null)
            {
                ...
                IFeature mostUpstreamArrow = null;
                double nearestDistance = laneLine.Length;
                //不是删除导向箭头，而是更新车道最上游的导向箭头，改为指前行方向的导向箭头
                while (feaArrow != null)
                {
                    //不使用这个， Convert.ToInt32(feaArrow.get_Value(feaArrow.Fields.FindField(SerialNm))) == 0
                    //利用几何特点
                    double distanceBetweenArrowLaneFromPoint = PointHelper.GetDistance2Point(laneLine.FromPoint,feaArrow.Shape as IPoint);
                    if (distanceBetweenArrowLaneFromPoint < nearestDistance)
                    {
                        nearestDistance = distanceBetweenArrowLaneFromPoint;
                        mostUpstreamArrow = feaArrow;
                    }
                    feaArrow = cursorArrow.NextFeature();
                }

                //车道最上游的导向箭头的，改为直行即可
                if (mostUpstreamArrow != null)
                {
                    setArrowAsGeneralDireation(mostUpstreamArrow);
                }
                else
                {
        ...
        public void CreateTurnArrowForEntranceLane(IFeature laneFeature)
        {
            ...
            //根据车道的长度判断是生成2 个还是 3个导向箭头
            int arrowCountInEnd = ARROW_COUNT_IN_SHORT_LANE;
            if (laneLine.Length > LENGHT_LONG_OR_SHORT)
            {
                arrowCountInEnd = ARROW_COUNT_IN_LONG_LANE;
            }
            ...
            for (int i = 0; i < arrowCountInEnd; i++)
            {
                ...
                IFeature arrowFea = CreateArrow(arrowEty, arrowPnt2);
