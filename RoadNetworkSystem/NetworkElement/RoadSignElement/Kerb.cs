...
namespace RoadNetworkSystem.NetworkElement.RoadSignElement
{
    class Kerb
    {
        ...
        public int KerbID;
        public Kerb(IFeatureClass pFeaClsKerb, int kerbID)
        {
            FeaClsKerb = pFeaClsKerb;
            KerbID = kerbID;
        }

        public KerbEntity GetEntity(IFeature pFeature)
        {
            KerbEntity bounEty = new KerbEntity();
            if (pFeature != null)
            {
        ...
        public IFeature CreateKerb(KerbEntity kerbEty, IPoint pnt)
        {
        ...
        public IFeature GetKerbByArcAndSerial(int arcID, int serial)
        {
            ...
            Conn.Open();
            string readStr = String.Format("select * from {0} where {1} = {2} and {3} = {4}", KerbEntity.KerbName, ArcIDNm, arcID, SerialNm, serial);
