...
namespace RoadNetworkSystem.NetworkElement.RoadSignElement
{
    class StopLine
    {
        ...
        public StopLine(IFeatureClass pFeaClsStopLine, int stopLineID)
        {
            FeaClsStopLine = pFeaClsStopLine;
            BouddaryID = stopLineID;
        }

        public StopLineEntity GetEntity(IFeature pFeature)
        {
            StopLineEntity bounEty = new StopLineEntity();
            if (pFeature != null)
            {
        ...
        public IFeature CreateStopLine(StopLineEntity bounEty, IPolyline line)
        {
